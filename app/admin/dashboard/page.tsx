...
const fromRow = (row: QuestionRow): Question => ({
  id: row.id,
...
const toRow = (question: Omit<Question, "id">) => ({
  category: question.category,
...
const toSampleRow = (question: Question) => ({
  id: question.id,
  category: question.category,
  question: question.question.trim(),
  option_a: question.options[0].trim(),
...
export default function Dashboard(){
  ...
  useEffect(()=>{
    const load=async()=>{
      ...
      if(queryError){setError(`Gagal memuat soal: ${queryError.message}`);setLoading(false);return;}
      setQuestions(data?.length?(data as QuestionRow[]).map(fromRow):[]);
      setLoading(false);
    };
    load();
  },[router]);
  ...
  const handleSubmit=async(e:React.FormEvent<HTMLFormElement>)=>{
    e.preventDefault();
    if(!qText.trim()||qOpts.some(o=>!o.trim())){alert("Isi semua field!");return;}
    const draft={category:qCat,difficulty:qDiff,question:qText,options:[...qOpts],correct:qCorrect} as Omit<Question,"id">;
    setSaving(true);setError("");
    if(!supabase){setError("Supabase belum dikonfigurasi.");setSaving(false);return;}
    const result=editing
      ? await supabase.from("questions").update(toRow(draft)).eq("id",editing.id).select("*").single()
      : await supabase.from("questions").insert(toRow(draft)).select("*").single();
    if(result.error){setError(`Gagal menyimpan soal: ${result.error.message}`);setSaving(false);return;}
    const saved=fromRow(result.data as QuestionRow);
    setQuestions(current=>editing?current.map(item=>item.id===saved.id?saved:item):[...current,saved]);
    setSaving(false);closeForm();
  };
  const deleteQ=async(id:number)=>{
    if(!confirm("Hapus soal ini?"))return;
    if(!supabase){setError("Supabase belum dikonfigurasi.");return;}
    const {error:deleteError}=await supabase.from("questions").update({is_active:false}).eq("id",id);
    if(deleteError){setError(`Gagal menghapus soal: ${deleteError.message}`);return;}
    setQuestions(current=>current.filter(item=>item.id!==id));
  };
  const syncSampleQuestions=async()=>{
    if(!supabase){setError("Supabase belum dikonfigurasi.");return;}
    if(!confirm(`Update ${sampleQuestions.length} soal dari sampleQuestions.ts?`))return;
    setSaving(true);setError("");
    const {data,error:syncError}=await supabase.from("questions").upsert(sampleQuestions.map(toSampleRow),{onConflict:"id"}).select("*");
    if(syncError){setError(`Gagal update soal sample: ${syncError.message}`);setSaving(false);return;}
    if(data)setQuestions((data as QuestionRow[]).map(fromRow));
    setSaving(false);
  };
  ...
  return <main className="sky relative min-h-screen p-5 md:p-8 overflow-hidden">
   ...
   <div className="relative z-10 max-w-6xl mx-auto">
    ...
    {error&&<div className="mb-4 border-2 border-red-500 bg-red-100 p-3 font-bold text-red-800">⚠ {error}</div>}
    {loading&&<div className="mb-4 border-2 border-yellow-500 bg-yellow-100 p-3 font-bold text-yellow-900">Memuat soal dari database...</div>}
     ...
     <div className="flex flex-col md:flex-row gap-3 items-center justify-between">
      ...
      <div className="flex gap-2">
       <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="🔍 Cari soal..." className="pixel-input w-48 text-sm" />
        <motion.button whileHover={{scale:1.05}} whileTap={{scale:.95}} onClick={syncSampleQuestions} disabled={saving} className="pixel-button rounded bg-emerald-500 text-white px-4 py-3 font-bold disabled:opacity-60">↻ Update Sample</motion.button>
     ...
     <motion.div initial={{scale:.8,opacity:0,y:50}} animate={{scale:1,opacity:1,y:0}} exit={{scale:.8,opacity:0,y:50}} transition={{type:"spring",stiffness:300,damping:20}} className="relative z-10 w-full max-w-2xl pixel-border rounded bg-white/50 p-8 overflow-y-auto max-h-[90vh] backdrop-blur-md">
      <div className="flex items-center justify-between mb-4">
       ...
       <div className="flex gap-3 pt-4 border-t">
        <motion.button type="submit" disabled={saving} whileHover={{scale:1.03}} whileTap={{scale:.97}} className="pixel-button rounded bg-indigo-500 text-white px-6 py-3 font-bold flex-1 disabled:opacity-60">💾 {saving?"MENYIMPAN...":editing?"UPDATE":"SIMPAN"}</motion.button>
